...
Скрипт применяет миграции и перезапускает приложение.

Откройте в браузере: http://127.0.0.1:8008

Документация REST API (Swagger): http://127.0.0.1:8008/api/docs
...
Настройки хранятся в таблице `settings` с полями:
...
Форма доступна в UI: `/settings`, API — `/api/settings`.
